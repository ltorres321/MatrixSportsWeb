...
import { GAMES, scalePosition, type GameStat, type GameStatSide } from "@/lib/gameStats";
  ...
  return (
    <div className="game-hero">
      <div className="matchup-status" style={{ justifyContent: "center", marginBottom: "1.5rem" }}>
        ...
        {game.status === "final" && <span className="tag final">FINAL</span>}
        {game.status === "preview" && <span className="tag preview">{game.kickoff}</span>}
      </div>
      ...
      {game.liveNote && <div className="game-hero-meta" style={{ marginTop: "0.5rem" }}>{game.liveNote}</div>}
    </div>
  );
}
...
function MarginSection({ game }: { game: GameStat }) {
  ...
  return (
    <div className="stat-section">
      ...
      <p className="stat-sub">How often each outcome happened across every simulated version of this game.</p>
  ...
  return (
    <div className="stat-section">
      ...
      <p className="stat-sub">
        Negative means {teamDisplay(favored.alias)} lost by that many points. The shaded band is the 25th&ndash;75th
    ...
    <div className="stat-section">
      ...
      <div className="result-compare">
        <div className="compare-card">
          ...
          <div className="value">
            {winnerName.split(" ").pop()} {projected}%
          </div>
        </div>
...
export default function GameDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const game = GAMES[id] ?? GAMES["kc-buf"];
  ...
  return (
    <>
      ...
      <footer className="site-footer">
        <p>
          Placeholder page — every number above is hardcoded demo data for layout purposes, not the output of the
          prediction model. Real per-game stats already exist in the live database and will be wired in once the
          site&apos;s design is final.
        </p>
      </footer>
    </>
  );
